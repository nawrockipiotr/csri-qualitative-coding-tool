...
async function runAutoCoding() {
  ...
  if (autoCancelled) { renderCodingView(); return; }
  ...
  try {
    ...
    for (const line of lines) {
      if (line.toUpperCase().startsWith('THEME:') || line.toUpperCase().startsWith('TEMAT:')) {
        ...
        const themeCodes = line.split(':').slice(1).join(':').split(',').map(c => c.trim()).filter(c => state.codebook[c]);
  ...
  try {
    ...
    for (const line of lines) {
      if (line.toUpperCase().startsWith('DIMENSION:') || line.toUpperCase().startsWith('WYMIAR:')) {
        ...
        const dimThemes = line.split(':').slice(1).join(':').split(',').map(th => th.trim()).filter(th => state.themes[th]);
        if (dimThemes.length) {
